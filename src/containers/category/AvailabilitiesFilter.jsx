...
const AvailabilitiesFilter = ({action, dispatch}) => {
    ...
    useEffect(() => {
        let optionsValue = Object.keys(options).filter(key => options[key].isChecked);
        ...
        <>
            <FormGroup>
                <FormControlLabel  control={
                    <Checkbox checked={isSearchAll} onClick={(e) => setIsSearchAll(e.target.checked)} />
                } label="Search all availabilities?"/>
                <motion.div layout className="pl-2">
                    <AnimatePresence>
                    {!isSearchAll && (Object.keys(options).map((key, index) => (
                        <motion.div
                            key={key} 
                            initial={{
                                opacity: 0
                        ...
                        >
                            <FormControlLabel control={
                                <Checkbox checked={options[key].isChecked} onClick={() => {
