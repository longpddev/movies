...
const StarRateItem = ({ children, onChange, onClick }) => {
  return (
...
const StarRate = ({ rateDefault = 0, onChangeRate }) => {
...
const MiniTab = ({ tabName, children, tabActive }) => {
  return tabActive === tabName ? children : null
}

const GotoRoot = ({ onClick }) => {
  return (
    <div>
      <button
        onClick={onClick}
...
// type: tv_show, movies
const CardMoreAction = ({ media_id, type }) => {
      ...
      >
        <AddCircleOutlineIcon
          className={`text-white ${
            openMoreAction ? "text-sky-400" : "hover:text-sky-400"
          }`}
        />
      </div>
      {openMoreAction && (
        <div className="absolute inset-0 w-full h-full z-10 flex items-center overflow-hidden justify-center">
          ...
          >
            <ul className="overflow-hidden rounded-md bg-white min-w-[80%]">
              ...
              <ActionItem onClick={handleAddFavorite}>
                <FavoriteIcon
                  sx={{ fontSize: "18px", marginRight: "7px" }}
              ...
              <ActionItem onClick={handleAddWatchlist}>
                <BookmarkIcon
                  sx={{ fontSize: "18px", marginRight: "7px" }}
              ...
              <ActionItem onClick={() => setTabActive("rating")}>
                <StarIcon
                  sx={{ fontSize: "18px", marginRight: "7px" }}
