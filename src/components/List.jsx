    ...
    <div className={clsx('c-container' ,className)}>
      <div className="flex mb-5">
        ...
        <div className="flex border border-gray-600 rounded-full">
          <button className="bg-white outline-0 border-0 py-1 px-5 relative bg-transparent" onClick={() => setFilter('tv')}>
            {filter === 'tv' && (
              <motion.div className="absolute inset-0 rounded-3xl w-full bg-cyan-900 z-[-1]" layoutId={id}></motion.div>
            )}
            <span className={clsx('z-[2] font-bold', {
              'text-cyan-400': filter === 'tv'
            })}>On Tv</span>
          </button>
          <button className="bg-white outline-0 border-0 py-1 px-5 relative bg-transparent" onClick={() => setFilter('theatres')}>
            {filter === 'theatres' && (
              <motion.div className="absolute inset-0 rounded-3xl w-full bg-cyan-900 z-[-1]" layoutId={id}></motion.div>
            )}
            <span className={clsx('z-[2] font-bold', {
