                        ...
                        <div className="grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-5 ">
                            {data.results.map((item) => (
                                ...
                                <div className="h-full relative hover:bottom-1 flex flex-col shadow-md border border-gray-300 rounded-sm hover:shadow-lg">
                                    <img src={getImage(item.profile_path)} title={item.name} alt={item.name} width="100%" height="270" className="object-cover block w-full " />
                                    <div className="px-3 py-2 bg-white h-full">
                                        <p className="font-bold text-xl">{item.name}</p>
                                        <p className="text-gray-400 text-sm limit-line-1">
                                            {item?.known_for?.map(i => i.original_title).filter(i => i?.length > 0).join(', ')}
                                        </p>
                                    </div>
                                </div>
                                </NavLink>
