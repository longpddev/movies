...
const Banner = ({ movieId }) => {
    ...
    return (
        <Fetching
            ...
            data={data}
            render={() => (
                <div className="detail-banner" style={{ backgroundImage: `url(${getImage(data.backdrop_path)})` }}>
                    <div className="detail-banner__inner">
                        <div className="c-container">
                            <div className="flex flex-wrap py-8">
                                <div className="sm:w-1/4 w-full">
                                    <img src={getImage(data.poster_path)} alt="" className="w-full rounded-lg" />
                                </div>
                                <div className="sm:w-3/4 sm:pl-8 w-full flex items-center">
                                    <div>
                                        <h1 className="text-3xl text-white"><strong>{data.title}</strong> ({moment(new Date(data.release_date)).format('YYYY')})</h1>
                                        <p className="text-white text-sm space-x-3">
                                            ...
                                            <span>{data.runtime > 60 ? `${Math.floor(data.runtime / 60)}h${data.runtime % 60}m` : `${data.runtime}m`}</span>
                                        </p>
